...
df = pd.read_csv('results/exp1_stress_results.csv', low_memory=False)
...
summary = pd.concat([run_n, sent_per_run, recv_per_run, recv_bytes_per_run], axis=1).dropna()
summary['PDR'] = summary['received'] / summary['sent']
summary['loss_ratio'] = 1 - summary['PDR']
summary['throughput_bps'] = (summary['received_bytes'] * 8) / 60.0
...
final.to_csv('results/exp1_stress_summary.csv', index=False)
print("\nSaved to results/exp1_stress_summary.csv")
